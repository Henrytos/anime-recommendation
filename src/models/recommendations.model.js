...
function findManyRecommendations(userId) {
  const query = `SELECT title, image_url, anime_id FROM users_recommendations WHERE user_id = ${userId} ORDER BY created_at DESC;`;
  ;
...
function findByUserMetricsMappings(userId) {
  ...
  SELECT
    ...
    GROUP BY animes.target_audience, quiz_result.fk_user_id
